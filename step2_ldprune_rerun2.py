...
# This script reruns LD pruning and produces new set of PLINK files using PLINK AGAIN - input = filtered and LD pruned (twice) NeuroGAP PLINK files
...
# perform pruning with a window size of 200 variants, sliding across the genome with step size of 100 variants at a time, and filter out any SNPs with LD r^2 > 0.1
...
if __name__ == '__main__':
    ...
    b = hb.Batch(backend=backend, name='perform LD pruning and subset PLINK files - run 3')  # define batch

    plink_files = b.read_input_group(**{
        'bed': 'gs://neurogap-bge-imputed-regional/mary/khat_gwas/intermediate_files/NeuroGAP_filtered_ldpruned_run2.bed',
        'bim': 'gs://neurogap-bge-imputed-regional/mary/khat_gwas/intermediate_files/NeuroGAP_filtered_ldpruned_run2.bim',
        'fam': 'gs://neurogap-bge-imputed-regional/mary/khat_gwas/intermediate_files/NeuroGAP_filtered_ldpruned_run2.fam'})

    # get file size
    plink_size = get_file_size('gs://neurogap-bge-imputed-regional/mary/khat_gwas/intermediate_files/NeuroGAP_filtered_ldpruned_run2.bed')
    storage_size1 = round(plink_size + 10)

    # run Step 2
    run_ld_prune = ld_prune(b, plink_files, storage_size1)
    b.write_output(run_ld_prune.ofile, f'gs://neurogap-bge-imputed-regional/mary/khat_gwas/intermediate_files/NeuroGAP_filtered_ldpruned_run3')
    ...
    b.write_output(run_generate_files.ofile, f'gs://neurogap-bge-imputed-regional/mary/khat_gwas/intermediate_files/NeuroGAP_filtered_ldpruned_run3')
